...
import { getBlogPost, getBlogPosts, BlogPost as BlogPostType } from "@/api/blog"
    ...
    const fetchPost = async () => {
      ...
      try {
        ...
        const [postResponse, postsResponse] = await Promise.all([
          getBlogPost(id) as Promise<{ post: BlogPostType }>,
          getBlogPosts() as Promise<{ posts: BlogPostType[] }>
        ])
        ...
        const related = postsResponse.posts
          .filter(p => p._id !== id && p.category === postResponse.post.category)
          .slice(0, 3)
          ...
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedPosts.map((relatedPost) => (
                <Card key={relatedPost._id} className="group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 bg-white/80 backdrop-blur-sm border-0 shadow-lg overflow-hidden">
