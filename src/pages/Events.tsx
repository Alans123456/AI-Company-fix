...
export function Events() {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<RegistrationForm>()
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const now = new Date()
        const upcoming = response.events.filter(event => new Date(event.date) >= now)
        const past = response.events.filter(event => new Date(event.date) < now)
  ...
  const onSubmit = async (data: RegistrationForm) => {
    ...
    try {
      ...
      const response = await registerForEvent(selectedEvent._id, data) as { success: boolean; message: string }
  ...
  const EventCard = ({ event, isPast = false }: { event: Event; isPast?: boolean }) => (
    <Card className={`group hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 bg-white/80 backdrop-blur-sm border-0 shadow-lg overflow-hidden ${isPast ? 'opacity-75' : ''}`}>
      <div className="relative">
        ...
        <div className="absolute top-4 left-4">
          <Badge className={isPast ? "bg-slate-500" : "bg-gradient-to-r from-blue-600 to-indigo-600"}>
            {event.type}
          </Badge>
        </div>
      ...
      <CardContent>
        ...
        {event.speakers && event.speakers.length > 0 && (
          <div className="mb-4">
            <h4 className="font-semibold text-slate-900 dark:text-white mb-2">Speakers:</h4>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-600 dark:text-slate-300 max-w-3xl mx-auto">
            Join us for workshops, seminars, and networking events designed to help you stay ahead in technology
          </p>
        </div>
